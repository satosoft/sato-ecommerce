...
class UploadController extends Controller
{
    ...
    {
        return view('uploader.sts-uploader');
    }
    public function upload(Request $request)
    {
        ...
        if ($request->hasFile('sts_file')) {
            $upload = new Upload;
            $extension = strtolower($request->file('sts_file')->getClientOriginalExtension());
            ...
            if (isset($type[$extension])) {
                ...
                $arr = explode('.', $request->file('sts_file')->getClientOriginalName());
                ...
                if($extension == 'svg') {
                    ...
                    // Load the dirty svg
                    $dirtySVG = file_get_contents($request->file('sts_file'));

                    // Pass it to the sanitizer and get it back clean
                    $cleanSVG = $sanitizer->sanitize($dirtySVG);

                    // Load the clean svg
                    file_put_contents($request->file('sts_file'), $cleanSVG);
                }

                $path = $request->file('sts_file')->store('uploads/all', 'local');
                $size = $request->file('sts_file')->getSize();

                // Return MIME type ala mimetype extension
                $finfo = finfo_open(FILEINFO_MIME_TYPE);
                $testa=base_path('public/');
                // Get the MIME type of the file
                $file_mime = finfo_file($finfo, base_path('public/') . $path);

                $testb=base_path('public/');

                if ($type[$extension] == 'image' && get_setting('disable_image_optimization') != 1) {
                    try {
                        $img = Image::make($request->file('sts_file')->getRealPath())->encode();
                        $height = $img->height();
                        $width = $img->width();
