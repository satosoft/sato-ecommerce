...
class HomeController extends Controller
{
    ...
    public function login()
    {
        ...
        }else if(Route::currentRouteName() == 'admin.login'){
    ...
    public function productDetails(Request $request, $slug)
    {
        ...
        if ($detailedProduct != null && $detailedProduct->published) {
            ...
            $review_status = 0;
            if (Auth::check()) {
                $OrderDetail = OrderDetail::with(['order' => function ($q) {$q->where('user_id', Auth::id());}])->where('product_id', $detailedProduct->id)->where('delivery_status', 'delivered')->first();
